...
BottomPositive=function( aggr ) {
  ...
  pc1=aggrx[grep("Ig Control",rownames(aggrx)),] 
  pc2=aggrx[grep("Total Ig",rownames(aggrx)),]
...
Normalization_Method=function(Intensity,background,target,method) {
	...
	delNo=PBS_Bad[,ncol(target)] 
        ...
        if(method==1){                         #RLM 
             ...
             pc1=Intensity[grep("Ig Control",rownames(Intensity)),] 
             pc2=Intensity[grep("Total Ig",rownames(Intensity)),]
...
Pre_process=function(Elist,measure_method,koc,batch_method,norm_method) {
   ...
   Num=seq(1,nrow(flagdata)) 
   flagdata=cbind(flagdata,Num) 
       
   if(measure_method==1) {
      flagdata[,1]=Elist$rawdata[,2] 
      flagdata[,2]=Elist$rawdata[,3] 
      flagdata[,3]=Elist$rawdata[,3]-Elist$rawdata[,2] 
      flagdata[,4]=Elist$rawdata[,3]/Elist$rawdata[,2] 
      flagdata[,5]=Elist$rawdata[,3]-Elist$rawdata[,1] 
      flagdata[,6]=Elist$rawdata[,3]/Elist$rawdata[,1] 
   } else if(measure_method ==2) {
      flagdata[,1]=Elist$rawdata[,4] 
      flagdata[,2]=Elist$rawdata[,5] 
      flagdata[,3]=Elist$rawdata[,5] -Elist$rawdata[,4] 
      flagdata[,4]=Elist$rawdata[,5]/Elist$rawdata[,4] 
      flagdata[,5]=Elist$rawdata[,5]-Elist$rawdata[,1] 
      flagdata[,6]=Elist$rawdata[,5]/Elist$rawdata[,1] 
   } else { 
  ...
  for( i in 1:nrow(elist$snr)) { 
    for(j in 1:ncol(elist$snr)) { 
      if(elist$snr[i,j]< 0.05) { elist$ratio[i,j]=1.001 ;elist$signal[i,j]=0.001; elist$signal_neighbour[i,j]=0.001;elist$ratio_neighbour[i,j]=0.001} 
      if(elist$signal[i,j] <5) { elist$ratio[i,j]=1.001} 
    }
  } 
  
  ifelse(elist$ratio < 1.001,1.001, elist$ratio) 
...
server <- shinyServer(function(input, output,session) {
  ...
  getGPR <- reactive({
    ...
    if (is.null(inFile)){
      ...
      elist$No_sample=nrow(elist$target)  
      attach(alldata) 
      if(proteinType==532) { 
  ...
  #############################################   QC report   #####################################################                                  
    ...
    getControls <- reactive({
      ...
      if(n.controls <3) {
        ...
        rownames(pc_control)="Igcontrol"
        pc_anti=control[1,] 
        pc_anti=t(as.matrix(pc_anti)) 
        rownames(pc_anti)="anti-Igcontrol"
        
      }else {
        pc_anti=control[grepl("anti-Ig control", rownames(control)), ]
        pc_control=control[-grep("anti-Ig control",rownames(control)),]
...
#############################################   Data Preprocessing   #####################################################
  ...
  getMatrix <- reactive({
    ...
    colnames(data_matrix)[ncol(data_matrix)] <- "%SNR>3"
    #delete that row
    return(data_matrix)
  })
  ...
  #############################################   Differential Expression   #####################################################
   ...
   # Read in layour file from user
     getGroup <- reactive({
           if (is.null(input$fileLayout)){
            return(NULL)
           }
           layout = read.table(input$fileLayout[[1, 'datapath']], header=T, sep="\t")
           return(layout)
     })
     
     
     #read in expression matrix files from user
     getEpr <- reactive({
     
            if (is.null(input$fileMatrix)){
            return(NULL)
           }
           eprMatrix = read.table(input$fileMatrix[[1, 'datapath']], header=T, sep="\t")
           return(eprMatrix)  
      })
...
output$colUI<-renderUI({
  ...
  selectizeInput(inputId ="pal", label ="Select Color Palette",
                 choices = c('Heat (Sequential)'='heat.colors',
                             'Vidiris (Sequential)'="viridis",
